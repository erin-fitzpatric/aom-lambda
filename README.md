...
```bash
leaderboard-extractor$ sam build
```
...
```bash
leaderboard-extractor$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
leaderboard-extractor$ sam local start-api
leaderboard-extractor$ curl http://localhost:3000/
```
...
```bash
leaderboard-extractor$ sam logs -n HelloWorldFunction --stack-name leaderboard-extractor --tail
```
...
```bash
leaderboard-extractor$ cd hello-world
...
```bash
sam delete --stack-name leaderboard-extractor
```
